...
// Routes configures routes exposed by the hook listener API.
func Routes(handler *EventHandler) *mux.Router {
	...
	api.Use(tracer)
	api.Use(eventParser)
	api.Methods("POST").Path("/namespaces/{namespace}/workflows/{name}/trigger").Handler(repositoryEventHandler(handler))
